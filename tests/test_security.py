...
from across_edge_worker.security import assert_endpoint_allowed,prepare_isolated_target,run_bounded_process,scan_text_for_secrets,scrub_environment,validate_relative_path
...
class SecurityTests(unittest.TestCase):
 ...
 def test_environment_scrubs_sensitive_names(self):
  sensitive=["AWS_SECRET_ACCESS_KEY","CLOUDFLARE_API_TOKEN","STRIPE_SECRET_KEY","PRIVATE_KEY","WALLET_SEED"];clean=scrub_environment({"PATH":"/bin",**{n:"x" for n in sensitive}},home="/tmp/clean")
  self.assertFalse(any(n in clean for n in sensitive))
 ...
 def test_endpoint_exact_allowlist(self):
 ...
 def test_bounded_process_kills_process_group(self):
  with tempfile.TemporaryDirectory() as td:
