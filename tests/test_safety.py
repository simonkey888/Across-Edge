...
@pytest.mark.parametrize('flag',['SEND_RELAYS','SEND_TRANSACTIONS','SEND_SLOW_RELAYS','RELAYER_USE_INVENTORY_MANAGER','EXECUTOR_ENABLED','PROPOSER_ENABLED','DISPUTER_ENABLED','REBALANCER_ENABLED','SWAP_REBALANCER_ENABLED','NOMINATION_WRITES_ENABLED','REGISTRATION_WRITES_ENABLED'])
...
def test_keys_nonvoid_write_rpc_and_broadcaster_blocked():
 ...
 for m in ['eth_sendRawTransaction','eth_sendTransaction','personal_sendTransaction','wallet_sendTransaction']:
...
def test_sanitizer_redacts_url_and_tokens():
 text=sanitize_text('https'+chr(58)+'//'+'user'+chr(58)+'pass'+'@example.com/rpc?key=abcdef Authorization: '+'Bearer '+'abcdefghijklmnop');assert 'user' not in text and 'pass' not in text and 'key=' not in text and 'abcdefghijklmnop' not in text;assert 'https://example.com/rpc' in text
