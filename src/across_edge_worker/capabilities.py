...
SECRET_TX_FIELDS={"signature","r","s","v","rawtransaction","signedtransaction","privatekey","mnemonic","seed","seedphrase","secretkey"}
@dataclass
class CapabilityContext:
 target:Path;allowed_paths:tuple[str,...];store:LifecycleStore;execution_id:str;allowed_endpoints:tuple[str,...];allowed_chain_ids:tuple[int,...];deadline_monotonic:float|None=None;cancel_marker:Path|None=None
 def guard(self)->None:
  if self.cancel_marker is not None and self.cancel_marker.exists():raise InterruptedError("worker_cancelled")
  if self.deadline_monotonic is not None and time.monotonic()>=self.deadline_monotonic:raise TimeoutError("worker_timeout")
 def remaining(self,ceiling:float=15)->float:
  self.guard()
  if self.deadline_monotonic is None:return ceiling
  return max(.05,min(ceiling,self.deadline_monotonic-time.monotonic()))
...
def chain_provenance(ctx,action):
 ctx.guard();endpoint=str(action["endpoint"]);expected=int(action["chain_id"]);key=f"chain-provenance:{expected}:{endpoint}";request={"endpoint":endpoint,"chain_id":expected,"methods":["eth_chainId","eth_blockNumber","eth_getBlockByNumber"]};cached=ctx.store.cached_read(ctx.execution_id,key,request)
 if cached is not None:return cached
 evidence=ReadOnlyRpcClient(endpoint,ctx.allowed_endpoints,ctx.allowed_chain_ids,timeout=ctx.remaining()).provenance().to_dict();ctx.guard()
...
def validate_unsigned_transaction(tx,allowed_chain_ids):
 for key in tx:
  ...
  if normalized in SECRET_TX_FIELDS:raise ValueError("signed_or_secret_transaction_material_forbidden")
...
def static_check(ctx,check):
 ctx.guard();kind=str(check.get("kind") or "");relative=str(check.get("path") or "");path=validate_relative_path(ctx.target,relative,ctx.allowed_paths,must_exist=True)
