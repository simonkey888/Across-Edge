...
class ReadOnlyRpcClient:
    def __init__(self,url:str,allowed_urls:tuple[str,...],allowed_chain_ids:tuple[int,...],*,timeout:int=15):
        assert_endpoint_allowed(url,allowed_urls); self.url=url; self.allowed_urls=allowed_urls; self.allowed_chain_ids=set(allowed_chain_ids); self.timeout=timeout
        self._opener=urllib.request.build_opener(NoRedirect(),urllib.request.HTTPSHandler(context=ssl.create_default_context())); self.calls=0
    def call(self,method:str,params:list[Any])->Any:
        ...
        request=urllib.request.Request(self.url,data=body,headers={"Content-Type":"application/json","User-Agent":"across-edge-atm-worker/1.0"},method="POST")
        try:
