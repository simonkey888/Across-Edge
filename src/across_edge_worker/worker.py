...
def run_job(job_path:Path,state_dir:Path,output_dir:Path):
 ...
 try:
  ...
  checks=[static_check(ctx,c) for c in job.deterministic_checks];ctx.guard();store.receipt(eid,"checks-completed","PROGRESS",{"objective":"deterministic_checks","count":len(checks),"checks_hash":sha256_json(checks)})
