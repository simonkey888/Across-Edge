...
from .security import prepare_isolated_target,scan_tree_for_secrets,sha256_file,snapshot_tree_hash
def _source_sha():
...
def run_job(job_path:Path,state_dir:Path,output_dir:Path):
    ...
    source_sha=_source_sha();store=LifecycleStore(state_dir/"worker.sqlite");eid=store.begin(job_id=job.job_id,lease_id=job.work_lease_id,scope_hash=job.scope_hash,source_sha=source_sha);existing={x["receipt_key"]:x for x in store.list_receipts(eid)}
    ...
    _maybe_crash("after_ack");project_root=Path(__file__).resolve().parents[2];before=snapshot_tree_hash(project_root);workdir=state_dir/"executions"/eid;store.advance(eid,"PREPARE_ISOLATED_TARGET");target=prepare_isolated_target(job.target_repository,job.target_base_sha,workdir);store.receipt(eid,"target-prepared","PROGRESS",{"objective":"isolated_target_exact_sha","target":str(target),"target_base_sha":job.target_base_sha,"project_hash_before":before});ctx=CapabilityContext(target,job.allowed_paths,store,eid,job.allowed_read_endpoints,job.allowed_chain_ids);store.advance(eid,"WORK");outputs=[];patches=[];chain_refs=[]
    for index,action in enumerate(list(job.structured_requirements.get("actions",[]))):
        ...
        outputs.append({"capability":cap,"observed":value});store.receipt(eid,f"progress-{index+1}","PROGRESS",{"objective":cap,"index":index+1,"output_hash":sha256_json(value)})
    checks=[static_check(ctx,c) for c in job.deterministic_checks];store.receipt(eid,"checks-completed","PROGRESS",{"objective":"deterministic_checks","count":len(checks),"checks_hash":sha256_json(checks)})
    findings=scan_tree_for_secrets(target)
