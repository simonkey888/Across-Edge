...
SECRET_PATTERNS = (
    ...
    re.compile(r"\b(?:sk|rk|pk)_(?:live|prod)_[A-Za-z0-9_-]{12,}\b"),
...
def scrub_environment(source: dict[str,str]|None=None, *, home: str|None=None)->dict[str,str]:
    ...
    clean["HOME"]=home or tempfile.mkdtemp(prefix="across-edge-worker-home-")
    clean["GIT_CONFIG_NOSYSTEM"]="1"; clean["GIT_TERMINAL_PROMPT"]="0"; clean["GIT_OPTIONAL_LOCKS"]="0"
    return clean
...
def scan_text_for_secrets(text:str)->list[str]:
    return [f"secret_pattern_{i}" for i,p in enumerate(SECRET_PATTERNS) if p.search(text)]
...
def prepare_isolated_target(target_repository:str,target_base_sha:str,workdir:Path)->Path:
    ...
    if target.exists():
        ...
        return target
    workdir.mkdir(parents=True,exist_ok=True); env=scrub_environment(); assert_no_sensitive_environment(env)
    subprocess.run(["git","-c","core.hooksPath=/dev/null","clone","--no-checkout","--filter=blob:none","--",target_repository,str(target)],env=env,check=True,timeout=120,stdin=subprocess.DEVNULL)
    subprocess.run(["git","-C",str(target),"-c","core.hooksPath=/dev/null","checkout","--detach",target_base_sha],env=env,check=True,timeout=120,stdin=subprocess.DEVNULL)
    subprocess.run(["git","-C",str(target),"config","core.hooksPath","/dev/null"],env=env,check=True)
    observed=subprocess.check_output(["git","-C",str(target),"rev-parse","HEAD"],text=True).strip()
    if observed!=target_base_sha: raise ValueError("target_sha_mismatch")
    return target

def run_bounded_process(command:list[str],*,cwd:Path,timeout:float,env:dict[str,str]|None=None)->subprocess.CompletedProcess[str]:
    safe_env=scrub_environment(env); assert_no_sensitive_environment(safe_env)
    process=subprocess.Popen(command,cwd=cwd,env=safe_env,stdin=subprocess.DEVNULL,stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True,start_new_session=True)
    try: stdout,stderr=process.communicate(timeout=timeout)
    except subprocess.TimeoutExpired:
        try: os.killpg(process.pid,signal.SIGTERM); process.communicate(timeout=3)
        except subprocess.TimeoutExpired: os.killpg(process.pid,signal.SIGKILL); process.communicate()
        raise TimeoutError("bounded_process_timeout")
