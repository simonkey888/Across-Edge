...
def sanitize_text(value:object)->str:
    text=str(value)
    text=re.sub(r'https?://[^\s]+',lambda m:sanitize_endpoint(m.group(0)),text)
    return _SECRET_RE.sub(lambda m:(m.group(1) or m.group(4) or '')+'<REDACTED>',text)
def validate_shadow_environment(env:Mapping[str,str]|None=None,argv:Sequence[str]=())->SafetyState:
    ...
    if secrets:raise SafetyViolation('ORDER-002 forbids real secret material: '+', '.join(secrets))
    enabled=tuple(sorted(k for k in PROHIBITED_TRUE_FLAGS if _truthy(env.get(k))))
    if enabled:raise SafetyViolation('LIVE_EXECUTION_PROHIBITED_BY_ORDER_002: '+', '.join(enabled))
    args=list(argv);wallet='void'
    if '--wallet' in args:
        i=args.index('--wallet')
        if i+1>=len(args):raise SafetyViolation('--wallet requires a value')
        wallet=args[i+1]
    if wallet!='void':raise SafetyViolation('ORDER-002 requires upstream --wallet void')
    return SafetyState(False,False,wallet,secrets)
def assert_read_only_rpc_method(method:str)->None:
    if method in LIVE_RPC_METHODS or method.startswith('eth_send') or method.startswith('personal_') or method.startswith('wallet_'):
        raise SafetyViolation(f'LIVE_EXECUTION_PROHIBITED_BY_ORDER_002: RPC method {method}')
    if method not in READ_ONLY_RPC_METHODS:raise SafetyViolation(f'RPC method is not allow-listed for ORDER-002: {method}')
class ProhibitedBroadcaster:
    def broadcast(self,*_args,**_kwargs):raise SafetyViolation('LIVE_EXECUTION_PROHIBITED_BY_ORDER_002')
