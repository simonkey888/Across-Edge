...
CANONICAL_REPO='across-protocol/relayer'
...
def safe_upstream_command(relayer_dir:str|Path,address:str='0x0000000000000000000000000000000000000000')->list[str]:return ['yarn','relay','--wallet','void','--address',address]
def safe_env(base:Mapping[str,str]|None=None)->dict[str,str]:
    env=dict(base or {})
    for key in list(env):
        ku=key.upper()
        if key in {'PRIVATE_KEY','MNEMONIC','SECRET','DISPATCHER_KEYS','ARWEAVE_WALLET_JWK','GCKMS_CONFIG','GOOGLE_APPLICATION_CREDENTIALS'} or any(x in ku for x in ('MNEMONIC','PRIVATE_KEY','SEED_PHRASE')):env.pop(key,None)
    ...
    env['ACROSS_EDGE_INSTRUMENTATION']='true';validate_shadow_environment(env,['--wallet','void']);return env
def run_shadow_once(relayer_dir:str|Path,extra_env:Mapping[str,str]|None=None)->subprocess.CompletedProcess[str]:
    verify_upstream_checkout(relayer_dir);env={k:v for k,v in os.environ.items() if k in {'PATH','HOME','USER','LOGNAME','TMPDIR','TEMP','TMP','TERM','LANG','LC_ALL','NODE_OPTIONS'}};env.update(extra_env or {});env=safe_env(env);cmd=safe_upstream_command(relayer_dir);validate_shadow_environment(env,cmd)
