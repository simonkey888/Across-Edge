    ...
    def run_once(self)->dict:
        ...
        if cursor and cursor['last_block_number'] is not None:
            ...
            if prior and cursor['last_block_hash'] and prior.get('hash','').lower()!=cursor['last_block_hash'].lower():
                rewind=max(self.spec.deployment_block,cursor['last_block_number']-self.reorg_depth+1);self.store.rewind_chain(self.spec.chain_id,rewind);cursor=None;reorg=True;self.store.bump_counter(self.run_id,'reorgs_detected');start=rewind
        ...
        for log in sorted(logs,key=lambda x:(int(x['blockNumber'],16),int(x.get('logIndex','0x0'),16))):
            ...
            try:
                if topic==FUNDS_DEPOSITED_TOPIC0.lower():self.observer.ingest_deposit(self.decode_deposit(log,origin_chain_id=self.spec.chain_id,block_timestamp=ts),self.run_id);accepted+=1
        ...
        if error_blocks:
            ...
            if certified>=self.spec.deployment_block:
                cb=block_cache.get(certified) or self._block(certified);self.store.set_cursor('spokepool',self.spec.chain_id,next_block,certified,cb.get('hash','') if cb else '')
            else:self.store.set_cursor('spokepool',self.spec.chain_id,next_block,None,None)
        else:self.store.set_cursor('spokepool',self.spec.chain_id,head+1,head,head_hash)
        ...
        return {'head':head,'from_block':start,'to_block':head,'logs':len(logs),'accepted':accepted,'errors':errors,'reorg':reorg,'cursor_next_block':self.store.get_cursor('spokepool',self.spec.chain_id)['next_block'],'unresolved_decode_gaps':len(gaps)}
    def run_continuous(self,stop_event,*,interval_s:float=2.0,max_backoff_s:float=30.0,on_cycle=None):
        failures=0;cycles=0
        while not stop_event.is_set():
            try:
                result=self.run_once();failures=0;cycles+=1
