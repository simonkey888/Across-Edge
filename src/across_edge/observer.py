...
class Observer:
    ...
    def ingest_deposit(self,d:DepositEvent,run_id:str,*,destination_time:int|None=None,trace_id:str|None=None,source_block_number:int|None=None,source_block_hash:str|None=None)->ShadowRecord:
        ...
        if existing:r=ShadowRecord(**existing)
            ...
            initial='other' if destination_time is None else classify_candidate(d,self.relayer_address,destination_time)
            r=ShadowRecord(3,run_id,d.key,d.origin_chain_id,d.deposit_id,d.destination_chain_id,d.input_token,d.output_token,d.input_amount,d.output_amount,d.exclusive_relayer,d.exclusivity_deadline,initial,trace_id=trace_id,decision_destination_time=destination_time,deposit_block=d.block_number)
    ...
    def _transition(self,r:ShadowRecord,d:DepositEvent,destination_time:int,*,source_chain_id:int|None=None,source_block_number:int|None=None,source_block_hash:str|None=None):
    ...
    def refresh_candidate_states(self,run_id:str,destination_chain_id:int,destination_time:int,*,source_block_number:int|None=None,source_block_hash:str|None=None):
    ...
    def ingest_fill(self,f:FillEvent,run_id:str,*,observed_monotonic_ns:int|None=None,observed_wall_utc:str|None=None)->bool:
        now=perf_counter_ns() if observed_monotonic_ns is None else observed_monotonic_ns;wall=utc_now() if observed_wall_utc is None else observed_wall_utc
        observed=f.with_observation(now,wall);inserted=self.store.insert_fill(observed)
        if inserted:self.reconcile_deposit(run_id,f.key)
        return inserted
    def reconcile_deposit(self,run_id:str,key:str)->None:
        rows=self.store.shadow_for_deposit(run_id,key)
        if not rows:return
        fills=self.store.fills_for_deposit(key);competitive=[f for f in fills if competitive_fill(f)];winner=competitive[0] if competitive else None;dep=self.store.deposit(key)
        for data in rows:
            for k,v in {'winner_relayer':'','winner_tx_hash':'','winner_block':None,'winner_log_index':None,'winner_fill_type':None,'tw_wall_utc':None,'tw_monotonic_ns':None,'winner_latency_ms':None,'shadow_headroom_ms':None}.items():data[k]=v
            if winner:
                data['winner_relayer']=winner['relayer'];data['winner_tx_hash']=winner['tx_hash'];data['winner_block']=winner['block_number'];data['winner_log_index']=winner.get('log_index');data['winner_fill_type']=winner.get('fill_type');data['tw_monotonic_ns']=winner.get('observed_monotonic_ns');data['tw_wall_utc']=winner.get('observed_wall_utc')
...
def competitor_scoreboard(deposits,fills,*,competitive_only=True):
    ...
    for relayer,pairs in grouped.items():
        ...
        out.append({'relayer':relayer,'fills_observed':len(pairs),'median_deposit_to_fill_ms':median(lat),'p10_ms':percentile(lat,.1),'p90_ms':percentile(lat,.9),'routes':sorted({f"{d['origin_chain_id']}->{d['destination_chain_id']}" for d,_ in pairs}),'fill_types':fill_type_counts([f for _,f in pairs])})
