...
class Observer:
    ...
    def ingest_deposit(self,d:DepositEvent,run_id:str,*,destination_time:int|None=None,trace_id:str|None=None,source_block_number:int|None=None,source_block_hash:str|None=None)->ShadowRecord:
        self.store.upsert_deposit(d);self.store.link_deposit(run_id,d.key,True);trace_id=trace_id or f'{d.key}:observer';existing=self.store.shadow_by_trace(run_id,trace_id)
    ...
    def reconcile_deposit(self,run_id,key):
        ...
        if not rows:return
        fills=self.store.fills_for_deposit(key,run_id);winner=next((f for f in fills if competitive_fill(f)),None);dep=self.store.deposit(key)
        for data in rows:
