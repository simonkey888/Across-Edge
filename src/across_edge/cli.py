...
def _observe(args):
    ...
    try:
        try:
            ...
            blocked=sanitize_text(exc);store.set_run_metadata(args.run_id,meta.finish(zero_write_rpc_proof='PASS',safety='PASS',real_network_read_only_smoke='EXPLICITLY_BLOCKED',blocker=blocked));print(json.dumps({'status':'EXPLICITLY_BLOCKED','reason':blocked},sort_keys=True));return 2
        store.set_run_metadata(args.run_id,meta.finish(zero_write_rpc_proof='PASS',safety='PASS',real_network_read_only_smoke='PASS'));print(json.dumps(results,indent=2,sort_keys=True));return 0
    finally:store.close()
def main(argv=None)->int:
    p=argparse.ArgumentParser(description=BANNER);sub=p.add_subparsers(dest='cmd',required=True);sub.add_parser('safety-check');report=sub.add_parser('report');report.add_argument('db');report.add_argument('run_id');report.add_argument('out');obs=sub.add_parser('observe-once');obs.add_argument('db');obs.add_argument('run_id');obs.add_argument('--chains',default='arbitrum,base');obs.add_argument('--backfill',type=int,default=128);verify=sub.add_parser('verify-upstream');verify.add_argument('path');args=p.parse_args(argv);print(BANNER);validate_shadow_environment(os.environ,['--wallet','void'])
    ...
    if args.cmd=='observe-once':return _observe(args)
    store=Store(args.db)
